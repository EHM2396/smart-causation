...
def get_cuentas_gasto(db: DB, user: AuthUser):
...
def get_metodos_pago(db: DB, user: AuthUser):
...
def get_cuenta(codigo: str, db: DB, user: AuthUser):
...
def post_cuenta(body: CuentaCreate, db: DB, user: AuthUser):
